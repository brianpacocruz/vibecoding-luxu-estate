            ...
            <div className="flex gap-4 overflow-x-auto hide-scroll pb-2 snap-x">
              {allImages.map((img, idx) => (
                <div key={idx} className="flex-none w-48 aspect-[4/3] rounded-lg overflow-hidden cursor-pointer opacity-70 hover:opacity-100 transition-opacity snap-start relative">
                  <Image
                    src={img}
                    alt={`${t.propertyDetail.galleryImage} ${idx + 1}`}
                    fill
            ...
            <div className="bg-white p-2 rounded-xl shadow-sm border border-mosque/5">
              {property.lat && property.lng ? (
                ...
                <div className="relative w-full aspect-[4/3] rounded-lg overflow-hidden bg-slate-100 flex items-center justify-center">
                  <span className="text-nordic-muted">{t.propertyDetail.mapNotAvailable}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 -mt-8">
        <div className="lg:col-span-8 space-y-8">
          <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5 mt-8">
            <h2 className="text-lg font-semibold mb-6 text-nordic-dark">{t.propertyDetail.propertyFeatures}</h2>
          ...
          <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
            <h2 className="text-lg font-semibold mb-4 text-nordic-dark">{t.propertyDetail.aboutHome}</h2>
            <div className="prose prose-slate max-w-none text-nordic-muted leading-relaxed">
              <p className="mb-4">
                {t.propertyDetail.aboutDesc1.replace("{location}", property.location)}
              </p>
              <p>
                {t.propertyDetail.aboutDesc2}
              </p>
            </div>
          </div>

          <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
            <h2 className="text-lg font-semibold mb-6 text-nordic-dark">{t.propertyDetail.amenities}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
              {t.propertyDetail.amenitiesList.map((amenity: string) => (
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-nordic-muted">
                  {t.propertyDetail.estimatedPaymentDesc.replace(
                    "{amount}",
                    `$${Math.floor(property.price * 0.005).toLocaleString()}`
                  )}
                </p>
              </div>
            </div>
